...
const CATEGORIES: readonly CategoryOption[] = [
    ...
    { key: "notebook", label: "Notebook / MacBook", icon: Laptop },
...
const BRANDS_BY_CATEGORY: Record<DeviceCategoryKey, string[]> = {
    ...
    notebook: ["Apple (MacBook)", "Dell", "Lenovo", "Acer", "HP", "Asus", "Outra Marca"],
    computador: ["Custom Desktop (PC Gamer)", "Dell OptiPlex", "HP", "Lenovo ThinkCentre", "All-in-One", "Outro"],
};

const COMMON_ISSUES: readonly string[] = [
    "Tela quebrada / Display sem imagem",
...
const SERVICE_TYPES = [
    ...
    },
    {
        ...
        desc: "Suporte para empresas com SLA e faturamento mensal.",
        icon: Building2,
    },
] as const;
...
export default function OrcamentoPage() {
    ...
    const [serviceType, setServiceType] = useState<
        "store_dropoff" | "courier_pickup" | "home_service" | "b2b_corporate"
    >("courier_pickup");
    ...
    const handleProceedToStep2 = () => {
        if (!brand) {
            setFormError("Por favor, selecione a marca do dispositivo.");
            return;
        }
        if (!model.trim()) {
            setFormError("Por favor, informe o modelo aproximado.");
            return;
        }
        if (!issue) {
            setFormError("Por favor, selecione o defeito apresentado.");
            return;
        }
        setFormError(null);
        setStep(2);
    ...
    const handleFinalSubmit = (e: React.FormEvent) => {
        ...
        submitLead(
            ...
            },
            {
                ...
                onError: (err) => {
                    setFormError(`Não foi possível enviar o orçamento: ${err.message}`);
                },
            }
        );
    };
    ...
    return (
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-16 space-y-8">
            ...
            {step === 2 && (
                <form onSubmit={handleFinalSubmit} className="bg-white rounded-2xl border border-zinc-200 p-6 sm:p-8 space-y-6 shadow-sm">
                    ...
                    <div className="space-y-4 pt-2">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label className="text-xs font-bold text-[#52525B] block mb-1">Nome Completo *</label>
                                <input
                                    type="text"
                                    required
                                    value={name}
                            ...
                            <div>
                                <label className="text-xs font-bold text-[#52525B] block mb-1">WhatsApp / Telefone com DDD *</label>
                                <input
                                    type="tel"
                                    required
                                    value={phone}
            ...
            {step === 3 && createdLead && (
                <div className="bg-white rounded-2xl border border-zinc-200 p-6 sm:p-10 text-center space-y-6 shadow-sm animate-in zoom-in-95">
                    ...
                    <div className="pt-2 flex flex-col sm:flex-row items-center justify-center gap-3">
                        <a
                            href={`https://wa.me/5511999998888?text=${whatsappPayloadMessage}`}
                            target="_blank"
